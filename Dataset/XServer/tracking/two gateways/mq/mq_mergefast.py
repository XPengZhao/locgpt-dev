...
class ClearableStack(LifoQueue):
            ...
            for _, _, body in self.channel.consume(self.rb_queue, auto_ack=True, inactivity_timeout=10):
                if body is not None:
                    ...
                    try:
                        ...
                        # print('queue %s size is:' % self.rb_queue, self.mg_queue.qsize())
                    except queue.Full:
                        self.channel.cancel()
                        self.channel.queue_purge(self.rb_queue)
                        self.connection.process_data_events()
...
def merge_data(hostname, port, lifoqueue):
    ...
    while True:
        ...
        if usrptimes.size != 0:
            ...
            if 'optitrack' in data_upload:
                ...
                lifoqueue.put({confidence:data_upload})
